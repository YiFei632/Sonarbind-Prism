...
class SonarBindPrismPretrain(ThirdGroundingDINO):
    ...
    def __init__(self,
                 ...
                 *args,
                 ...
                 pretrain_roi_spatial_scale=0.125,
                 **kwargs) -> None:
        ...
        self.pretrain_text_templates = pretrain_text_templates or ["a photo of a {}"]
    ...
    def _init_layers(self) -> None:
        ...
        # Pretrain mode: add RoI components
        if self.pretrain_mode:
            ...
            # RoI Contrastive Focal Loss
            self.pretrain_loss_fn = MODELS.build(dict(
                type='RoIContrastiveFocalLoss',
                alpha=0.25,
                gamma=2.0,
                temperature=0.07,
            ...
            self._class_names = None  # Cache for class names
    ...
    def encode_class_texts(self, class_names):
        """Encode class names using multiple text templates.

        Args:
            class_names (list): List of class names

        Returns:
            Tensor: Text features of shape (num_classes, embed_dims)
        """
        all_text_prompts = []
        for class_name in class_names:
            for template in self.pretrain_text_templates:
                all_text_prompts.append(template.format(class_name))

        # Encode all text prompts
        text_dict = self.language_model(all_text_prompts)
        text_feats = text_dict['embedded']  # (num_prompts, max_len, dim)
        text_mask = text_dict['text_token_mask']  # (num_prompts, max_len)

        # Map to embed_dims
        if self.text_feat_map is not None:
            text_feats = self.text_feat_map(text_feats)

        # Pass through text encoder only (no fusion)
        text_feats = self._forward_text_encoder_only(text_feats, text_mask)

        # Pool: take [CLS] token (first token)
        text_feats = text_feats[:, 0, :]  # (num_prompts, embed_dims)

        # Average over templates for each class
        num_classes = len(class_names)
        num_templates = len(self.pretrain_text_templates)
        text_feats = text_feats.view(num_classes, num_templates, -1).mean(dim=1)

        return text_feats

    def _forward_text_encoder_only(self, text_feats, text_mask):
        """Pass text through Text Self-Attention Encoder only (no fusion)."""
        query_pos = torch.zeros_like(text_feats)
        for layer in self.encoder.text_layers:
            text_feats = layer(text_feats, query_pos, key_padding_mask=~text_mask)
        return text_feats

    def _forward_visual_encoder_only(self, feat, feat_mask, feat_pos,
                                      spatial_shapes, level_start_index, valid_ratios):
        """Pass visual features through Visual Self-Attention Encoder only (no fusion)."""
        ...
        # Pass through visual layers only
    ...
    def extract_roi_features(self, visual_feats, gt_bboxes, spatial_shapes):
        """Extract RoI features using RoIAlign.

        Args:
            visual_feats: (B, N, D) encoder output
            gt_bboxes: List of (num_boxes, 4) tensors in xyxy format
            spatial_shapes: (num_levels, 2) tensor

        Returns:
            Tensor: RoI features of shape (total_boxes, D)
        """
        B, N, D = visual_feats.shape

        # Use only first level features
        H, W = spatial_shapes[0].tolist()
        level_0_size = H * W
        visual_feats_level0 = visual_feats[:, :level_0_size, :]

        # Reshape to spatial format: (B, D, H, W)
        visual_spatial = visual_feats_level0.view(B, H, W, D).permute(0, 3, 1, 2).contiguous()

        # Prepare RoI list for RoIAlign
        roi_list = []
        for batch_idx, bboxes in enumerate(gt_bboxes):
            if len(bboxes) > 0:
                batch_indices = torch.full((len(bboxes), 1), batch_idx,
                                          dtype=bboxes.dtype, device=bboxes.device)
                rois = torch.cat([batch_indices, bboxes], dim=1)
                roi_list.append(rois)

        if len(roi_list) == 0:
            return torch.empty((0, D), device=visual_feats.device)

        all_rois = torch.cat(roi_list, dim=0)

        # Apply RoIAlign
        roi_feats = self.roi_align(visual_spatial, all_rois)  # (total_boxes, D, 7, 7)

        # Global average pooling
        roi_feats = roi_feats.mean(dim=[2, 3])  # (total_boxes, D)

        return roi_feats

    def build_dynamic_gt_matrix(self, gt_labels, num_classes):
        """Build ground truth matrix for contrastive loss.

        Args:
            gt_labels: List of label tensors for each image
            num_classes: Number of classes

        Returns:
            Tensor: GT matrix of shape (total_boxes, num_classes)
        """
        all_labels = torch.cat(gt_labels, dim=0)  # (total_boxes,)
        total_boxes = len(all_labels)

        # Create one-hot encoding
        gt_matrix = torch.zeros(total_boxes, num_classes,
                               device=all_labels.device, dtype=torch.float32)
        gt_matrix[torch.arange(total_boxes), all_labels] = 1.0

        return gt_matrix

    def _loss_pretrain(self, batch_inputs, batch_data_samples):
        """Compute pretraining loss using RoI-text contrastive learning."""
        # Get class names - try multiple sources
        class_names = None

        # Try 1: Check first sample's metainfo
        ...
        # Try 2: Check text attribute
        elif hasattr(batch_data_samples[0], 'text') and batch_data_samples[0].text is not None:
            if isinstance(batch_data_samples[0].text, (tuple, list)):
                class_names = batch_data_samples[0].text
        # Try 3: Use cached class names
        elif self._class_names is not None:
            class_names = self._class_names
        # Try 4: Check dataset_meta
        ...
        if class_names is None:
            ...
            print(f"DEBUG: batch_data_samples[0] attributes: {dir(batch_data_samples[0])}")
            if hasattr(batch_data_samples[0], 'metainfo'):
                print(f"DEBUG: metainfo keys: {batch_data_samples[0].metainfo.keys()}")
            raise ValueError("Cannot find class names in batch_data_samples")
        ...
        # Encode class texts
        text_feats = self.encode_class_texts(class_names)  # (num_classes, D)
        ...
        # Pass through visual encoder only
        ...
        visual_feats = self._forward_visual_encoder_only(
            feat, feat_mask, feat_pos, spatial_shapes, level_start_index, valid_ratios
        )

        # Extract GT boxes and labels
        gt_bboxes = [ds.gt_instances.bboxes for ds in batch_data_samples]
        gt_labels = [ds.gt_instances.labels for ds in batch_data_samples]

        # Extract RoI features
        roi_feats = self.extract_roi_features(visual_feats, gt_bboxes, spatial_shapes)
        roi_feats = F.normalize(roi_feats, p=2, dim=1)

        # Compute similarity
        similarity = torch.mm(roi_feats, text_feats.t())  # (total_boxes, num_classes)

        # Build GT matrix
        gt_matrix = self.build_dynamic_gt_matrix(gt_labels, num_classes)

        # Compute loss
        loss = self.pretrain_loss_fn(similarity, gt_matrix)

        return {'loss_pretrain': loss}
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # Pretrain mode: use RoI-text contrastive learning
        if self.pretrain_mode:
