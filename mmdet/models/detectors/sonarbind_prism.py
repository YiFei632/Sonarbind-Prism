...
class SonarBindPrism(ThirdGroundingDINO):
    ...
    def _loss_pretrain(self, batch_inputs: Tensor, batch_data_samples: SampleList):
        ...
        if self.pretrain_modality == 'rgb':
            ...
            # 提取声纳特征（需要从batch_data_samples中获取）
            sonar_imgs = torch.stack([s.sonar_img for s in batch_data_samples])
            visual_feats = self.sonar_backbone(sonar_imgs)
            if isinstance(visual_feats, (tuple, list)):
                visual_feats = visual_feats[-1:]  # 只取最后一层

        # 2. pre_transformer处理
        encoder_inputs_dict, _ = self.pre_transformer(visual_feats, batch_data_samples)
