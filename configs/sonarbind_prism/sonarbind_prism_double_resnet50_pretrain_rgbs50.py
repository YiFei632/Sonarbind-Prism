...
lang_model_name = 'your/path/to/bert-base-uncased'  # 替换为实际路径
...
model = dict(
    ...
    backbone=dict(
        ...
        norm_eval=True,
        style='pytorch',
        init_cfg=dict(type='Pretrained', checkpoint='your/path/to/resnet50')
    ),
    sonar_backbone=dict(
        type='ResNet',
        depth=50,
    ...
    # ========== 新增：FFT Band0 滤波配置 ==========                                                                                                                                                 
    fft_band0_filter=dict(                                                                                                                                                                           
        radius_ratio=0.2,  # Band0外半径占最大半径的比例（推荐0.1-0.3）                                                                                                                              
        enabled=True,      # 是否启用滤波（设为False可快速对比实验）                                                                                                                                 
    ),                                                                                                                                                                                               
    # ============================================
    # Contrastive loss configuration for multi-modal alignment
    loss_sonar_image_weight=1.0,  # Weight for sonar-image contrastive loss
    loss_sonar_text_weight=1.0,   # Weight for sonar-text contrastive loss
...
# dataset settings
# 声纳数据集根目录
sonar_data_root = 'your/path/to/sonar_dataset/'
...
data_root = 'your/path/to/image_dataset/'
...
train_dataloader = dict(
    _delete_=True,
    batch_size=8,  # 双图像输入，减小batch_size
    num_workers=4,
...
load_from = 'your/path/to/sonarbind_prism'